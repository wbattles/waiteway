...
CREATE TABLE IF NOT EXISTS policies (
	...
	api_key_team_ids         TEXT NOT NULL DEFAULT '',
	api_keys                 TEXT NOT NULL DEFAULT '',
	require_user_auth        INTEGER NOT NULL DEFAULT 0,
	user_auth_team_ids       TEXT NOT NULL DEFAULT '',
...
CREATE TABLE IF NOT EXISTS route_access_teams (
	route_id INTEGER NOT NULL REFERENCES routes(id) ON DELETE CASCADE,
	team_id  INTEGER NOT NULL REFERENCES teams(id) ON DELETE CASCADE,
	PRIMARY KEY (route_id, team_id)
	);

CREATE TABLE IF NOT EXISTS route_edit_teams (
	route_id INTEGER NOT NULL REFERENCES routes(id) ON DELETE CASCADE,
	team_id  INTEGER NOT NULL REFERENCES teams(id) ON DELETE CASCADE,
	PRIMARY KEY (route_id, team_id)
	);
...
func runMigrations(db *sql.DB) error {
	columns := []string{
		...
		"ALTER TABLE policies ADD COLUMN api_key_team_ids TEXT NOT NULL DEFAULT ''",
		"ALTER TABLE policies ADD COLUMN require_user_auth INTEGER NOT NULL DEFAULT 0",
		"ALTER TABLE policies ADD COLUMN user_auth_team_ids TEXT NOT NULL DEFAULT ''",
	...
	indexes := []string{
		...
		"CREATE UNIQUE INDEX IF NOT EXISTS idx_teams_acronym ON teams(acronym)",
	}
...
func (s *Store) ListPolicies() ([]Policy, error) {
	...
	for rows.Next() {
		...
		var apiKeyTeamIDs string
		var apiKeys string
		var requireUserAuth int
		var userAuthTeamIDs string
		...
		policy.APIKeyTeamIDs = splitInts(apiKeyTeamIDs)
		policy.APIKeys = splitLines(apiKeys)
		policy.RequireUserAuth = requireUserAuth == 1
		policy.UserAuthTeamIDs = splitInts(userAuthTeamIDs)
